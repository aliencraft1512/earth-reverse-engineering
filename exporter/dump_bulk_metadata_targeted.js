...
function summarizeBulk(utils, bulk, bulkPath) {
  ...
  for (const relPath of relPaths) {
    ...
    entries.push({
      ...
      hasNode: flags === null ? null : utils.bulk.hasNodeAtIndex(bulk, index),
      hasBulkMetadata:
      ...
      textureFormat:
        ...
        null,
      metersPerTexel: bulk.metersPerTexel?.[index] ?? null,
      orientedBoundingBox: bulk.orientedBoundingBox?.[index] ?? null,
...
async function dumpFromLatLon(utils, convertLatLongToOctant, outDir, lat, lon, maxLevel, fetchNode) {
...
async function dumpFromKnownOctants(utils, outDir, octants, fetchNode) {
  const results = [];
  for (const octant of octants) {
    const clean = String(octant).trim();
    if (!clean) continue;
    const trace = await traceNodePath(utils, clean, outDir, fetchNode);
    results.push({
      octant: clean,
      final: trace.final,
    });
  ...
  await writeJson(path.join(outDir, "known_octants_results.json"), results);
}
...
async function main() {
  ...
  const outDir = path.resolve(
    args.out || path.join(__dirname, "downloaded_files", "metadata_targeted")
  );

  const lat = num(args.lat, null);
  const lon = num(args.lon, null);
  const maxLevel = num(args["max-level"], 18);
  const fetchNode = !!args["fetch-node"];
  const octants = toArray(args.octant);
  ...
  if (lat !== null && lon !== null) {
    await dumpFromLatLon(utils, convertLatLongToOctant, outDir, lat, lon, maxLevel, fetchNode);
  }

  if (octants.length) {
    await dumpFromKnownOctants(utils, outDir, octants, fetchNode);
  }
